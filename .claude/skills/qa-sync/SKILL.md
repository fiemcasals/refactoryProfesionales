...
- Si en `docs/scrum-manifest.json` ya hay un `testId` para esta combinación
  `requirementCode` + `title` (mapeado por una corrida anterior de cualquiera de los dos
  skills), actualizar ese test:
  ```bash
  ...
  ```
- Si no existe, crearlo con test + pasos juntos:
  ```bash
  ...
  ```
  y guardar el `id` devuelto en el manifest.
